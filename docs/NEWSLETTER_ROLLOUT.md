...
Slack 中人工触发 `opening-digest` 始终作为隔离测试运行，使用 run ID 生成唯一 `[TEST]` Customer.io 身份和 `[测试]` 微信标题；它不会与当天正式稿同名，也不会复用或改写正式稿。只有 cron 触发使用正式身份，防止人工测试意外成为正式发送。
