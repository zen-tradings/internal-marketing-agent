...
| WeChat render theme | `zen-wechat/zen-trading@5`, `assets/zen-trading.css`, and `RENDER_OPTS` | Theme files deploy with the repo; body `h2` headings are rasterized into numbered bilingual cards; blockquotes normalize to body size; the final HTML runs font, duplicate-source, and fixed-footer-order checks before publishing |
