...
| Translation configuration | `OPENROUTER_TRANSLATION_MODEL` / `OPENROUTER_TRANSLATION_REASONING_EFFORT` / `TRANSLATION_BATCH_CONCURRENCY` / `TRANSLATION_*` / document credentials / `DATALAB_*` in `.env` | Separates translation inference from original writing; production batch concurrency is capped at 2 and remains under the global OpenRouter limit; the remaining keys control sources, private documents, PDF pages, browser, parse quality, timeouts, and redirects |
