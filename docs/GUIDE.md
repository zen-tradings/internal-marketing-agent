...
| WeChat render theme | `zen-wechat/zen-trading@10`, `assets/zen-trading.css`, `assets/zen-section-heading-card.png`, and `RENDER_OPTS` | Theme files deploy with the repo; final HTML materializes list bullets/ordinals (including ordered-list start/value semantics) and prevents loose-list paragraphs from compounding the body font size; body `h2` headings are rasterized onto a clean card plate; the Chinese title matches body-copy visual size at article width, the English eyebrow stays smaller, and Chromium fits long bilingual copy within fixed safety margins before capture; blockquotes normalize to body size; the final HTML runs font, duplicate-source, and fixed-footer-order checks before publishing |
