...
③ Research + writing   src/core/analysis-v2.js + src/core/runner.js
   ...
   model receives only relevant evidence; production generic, translation,
   Opening Digest, and options-strategy writing use GLM 5.3 Flash. GLM 5.2's
   fact audit returns exact
...
| Translation fidelity/completeness/checkpoint | `src/workflows/translation-source-text.js` | Datalab multi-page root containers parsed in original order; three-way coverage validation across Poppler/Datalab/structured body; up to two globally governed batch workers; fragmented inputs adapt from 24 to 48 items while retaining the 8,000-character cap; immutable tokens are masked before untranslated-text detection; low-confidence numeric-format warnings skip model repair; explicit invariants retain at most two targeted repair rounds, per-unit checkpoints, and hard structure/asset gates |
| Translation execution and research trace | `src/workflows/translate-engine.js` + translation branch of `src/core/runner.js` | Uses an independent translation model/reasoning role (production default `high`; lower efforts failed live no-regression acceptance) and writes the manifest, strict-equivalence status, review candidates, plus content-free per-request latency/queue/provider/token/cost telemetry to the trace |
...
| Translation configuration | `OPENROUTER_TRANSLATION_MODEL` / `OPENROUTER_TRANSLATION_REASONING_EFFORT` / `OPENROUTER_TRANSLATION_MAX_TOKENS` / `TRANSLATION_BATCH_CONCURRENCY` / `TRANSLATION_*` / document credentials / `DATALAB_*` in `.env` | Separates translation inference from original writing; the dedicated completion budget (default 24000) keeps reasoning tokens from truncating visible output; production batch concurrency is capped at 2 and remains under the global OpenRouter limit; the remaining keys control sources, private documents, PDF pages, browser, parse quality, timeouts, and redirects |
...
| WeChat render theme | `zen-wechat/zen-trading@11`, `assets/zen-trading.css`, `assets/zen-section-heading-card.png`, and `RENDER_OPTS` | Theme files deploy with the repo; final HTML materializes list bullets/ordinals (including ordered-list start/value semantics) and prevents loose-list paragraphs from compounding the body font size; body `h2` headings are rasterized onto a clean card plate; the Chinese title matches body-copy visual size at article width, the English eyebrow stays smaller, and Chromium fits long bilingual copy within fixed safety margins before capture; blockquotes normalize to body size; math formulas are extracted before markdown parsing, rasterized to 3x transparent PNGs (MathJax + shared Chromium), restored as `<img>` nodes, and hard-gated for placeholder/TeX/MathJax residue before publishing; the final HTML runs font, duplicate-source, and fixed-footer-order checks before publishing |
...
| Opening Digest daily brief | `src/workflows/opening-digest.js` + Customer.io/WeChat/Discord channels | Generated 10:00 ET, target 10:15; the email sends/schedules first, then formal WeChat and Discord deliveries persist the same frozen payload in SQLite. The WeChat consumer removes source links/citations before GLM 5.3 Flash translation while preserving numbers, tickers, times, brands, structure, and semantic link labels; email/Discord links remain unchanged. An invalid dynamic title falls back to `今日开市要点`. The WeChat `media_id` is persisted immediately, transient failures retry up to eight times across restarts, and readback repairs update one draft instead of deleting/recreating it. Dedicated Customer.io/WeChat templates remain `zen-customerio/zen-research@8` / `zen-wechat/zen-trading@9`; formal titles use `动态标题（日报· YYYY-MM-DD）`, TEST titles retain `[测试]` and `MM-DD`, and the fixed Discord URL remains plain text above the survey/QR tail. Manual TEST runs never reach `#newsletter-feed`; failures in either derived channel never rewrite the email result, and a verified WeChat delivery posts one Slack success notice (title and media ID) via the notification outbox. |
