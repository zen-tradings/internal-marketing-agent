...
| 直译翻译/完整性/checkpoint | `src/workflows/translation-source-text.js` | Datalab 多页根容器原序解析，Poppler/Datalab/结构化正文三方覆盖校验；逐文本节点翻译、不可变 token、最多两轮定向修复、宽松复核例外、逐单元 checkpoint 及结构/资产硬门禁 |
